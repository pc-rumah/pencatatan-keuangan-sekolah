    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Tambah Tagihan</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('tagihan.store') }}" method="POST" data-parsley-validate>
                    @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select class="form-select" name="jenis_id" id="jenis_id" required>
                                ...
                                @foreach ($jenis as $item)
                                    <option value="{{ $item->id }}" @selected(old('jenis_id') == $item->id)>
                                        {{ $item->jenis_tagihan }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
